...
# Прості довідники: одна назва-колонка, без інших полів. Слаг у URL валідується
# проти цього білого списку, тому інтерполяція назв таблиці/колонки в SQL нижче безпечна.
_LOOKUP_TABLES = {
    "military-branches": {
...
def _lookup_rows(db: sqlite3.Connection, slug: str):
    ...
    return db.execute(
        f"SELECT {config['id_col']} AS id, {name_col} AS name "
...
def create_lookup_item(
    ...
    config = _lookup_config(slug)
    try:
        db.execute(f"INSERT INTO {config['table']} ({config['name_col']}) VALUES (?)", (name,))
        db.commit()
    except sqlite3.IntegrityError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return RedirectResponse(url="/settings", status_code=303)
