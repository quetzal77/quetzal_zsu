...
- **`army_corps`** — довідник корпусів; `founded_date` (дата заснування), `emblem_file`
  (емблема корпусу) та `command_id` (FK на `territorial_commands` — оперативне командування,
  якому підпорядкований корпус) — редагуються на плашці «Армійські корпуси» в `/settings`. FK лише
  в цей бік (від корпусу до ОК): одному ОК може підпорядковуватись кілька корпусів
- **`territorial_commands`** — довідник оперативних командувань; FK на `military_branches` (рід
  військ, якому підпорядковане ОК) і `military_branch_details`; `is_force` — позначка «сила»
  (структура на кшталт Морської авіації/Морської піхоти, а не звичайне ОК — див. `/zsu/{slug}`);
  на це посилаються і `brigades.territorial_command_id`, і `army_corps.command_id`
- **`troop_types`** — довідник типів родів військ; FK на `military_branches` (рід військ) і
  `military_branch_details`; `is_force` — та сама позначка «сила», що й у `territorial_commands`;
  редагується на плашці «Типи родів військ» в `/settings`
...
- Універсальний механізм для довідників (lookup-таблиць) у `app/routers/settings.py`:
  ...
  локацію/рід військ/деталі роду військ/оперативне командування) — додати нове поле довіднику здебільшого означає
...
- Типи `extra_cols`: `"date"`, `"location"` (select із локацій), `"branch"` (select із родів
  ...
  за `details_name` — напр. Роди військ), `"command"` (select із `territorial_commands` за
  `command_name` — напр. Армійські корпуси обирають ОК, якому підпорядковані), без `"type"` —
  звичайне текстове поле (файл/URL).
- `"wide": True` — плашка розкладається фіксованим грідом (`"grid_cols"`, за замовчуванням 4
  колонки; 5 — у Оперативних командувань і Армійських корпусів, 6 — у Типів родів військ, бо в них
  3-4 додаткові поля) і займає весь рядок (`grid-column: 1 / -1`, тому в двоколонковому гріді
  сторінки плашка автоматично опиняється на власному рядку без ручного заповнювача); без цього
  прапорця — звичайний flex-рядок на половину ширини сторінки (напр. Роди військ, Локації).
...
- Порядок плашок на сторінці — явний у `settings.html` (не просто порядок словника):
  Роди військ → Типи родів військ → Оперативні командування → Деталі родів військ →
  Армійські корпуси → Локації → Регіони → Типи з'єднань → решта довідників.
...
- **«Окремі сили» на сторінці роду військ (`/zsu/{slug}`):** плашки над «Оперативними
  командуваннями» об'єднують дві сутності з прапорцем `is_force = 1` — територіальні
  командування (`kind: "command"`, напр. Морська піхота у ВМС) і типи родів військ
  (`kind: "troop-type"`, напр. Морська авіація у ВМС, Зенітні ракетні війська й Радіотехнічні
  війська у Повітряних силах), відсортовані разом за українським алфавітом
  (`_ukrainian_sort_key` з `app/database.py`). `is_force`-командування більше НЕ показуються
  серед звичайних «Оперативних командувань» (`command_list` у `zsu_branch` в
  `app/routers/zsu.py` явно виключає їх) — лише в «Окремих силах».
  Кожен елемент фільтрує список з'єднань за своїм виміром: командні — за `commandId`
  (той самий вимір, що й «Оперативні командування», тому й корпуси реагують на їх вибір так само),
  типи родів військ — за окремим `troopTypeId` (корпуси на нього не зважають). JS у
  `zsu_branch.html` розрізняє кнопки по атрибуту (`data-command-id` / `data-troop-type-id`),
  `setupFilter()` бере лише кнопки з відповідним атрибутом.
  Кнопка переходу вглиб веде на `/zsu/{slug}/{command_id}` (`zsu_branch_command`) для командних
  елементів або `/zsu/{slug}/type/{type_id}` (`zsu_branch_troop_type`) для типів родів військ —
  обидва саб-роути використовують той самий шаблон `zsu_branch.html`, але фільтрують з'єднання й
  корпуси по своєму виміру (`territorial_command_id` чи `troop_type_id` + `military_branch_id`,
  бо тип роду військ не унікальний для роду військ — напр. «Зенітні ракетні війська» є і в
  Повітряних силах, і в ВМС).
...
- Перегляд (списки, деталі, структура ЗСУ) — публічний, без входу.
- **Статистика (`/stats`) публічною не є** — захищена через `Depends(require_login)`, і
  посилання на неї в навігації (`base.html`) показується лише залогіненим редакторам.
